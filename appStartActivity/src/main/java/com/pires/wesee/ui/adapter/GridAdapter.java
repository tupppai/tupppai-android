	...
	protected ViewGroup bindView(ViewGroup root, int pos) {
		...
		for (int i = 0; i < mColumns; i++) {
			...
			if (childCount == mColumns) {
				...
				// mColumns,
						(outSize.x - 2 * Utils.dpToPx(mContext, 4))
								/ mColumns,
						(outSize.x - 2 * Utils.dpToPx(mContext, 4))
								/ mColumns);
				// LinearLayout.LayoutParams.WRAP_CONTENT);
				...
				if (mColumns == 3) {
					if (index % mColumns == 1) {
						lp.setMargins(Utils.dpToPx(mContext, 4),
								Utils.dpToPx(mContext, 4),
								Utils.dpToPx(mContext, 4), 0);
					} else {
						// 增加上下边距
						lp.setMargins(0, Utils.dpToPx(mContext, 4),
								0, 0);
					}
				}
