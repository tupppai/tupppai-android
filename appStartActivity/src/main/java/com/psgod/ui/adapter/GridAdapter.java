	...
	protected ViewGroup bindView(ViewGroup root, int pos) {
		...
		for (int i = 0; i < mColumns; i++) {
			...
			if (childCount == mColumns) {
				...
				// mColumns,
						(outSize.x - 2 * com.psgod.Utils.dpToPx(mContext, 4))
								/ mColumns,
						(outSize.x - 2 * com.psgod.Utils.dpToPx(mContext, 4))
								/ mColumns);
				// LinearLayout.LayoutParams.WRAP_CONTENT);
				...
				if (mColumns == 3) {
					if (index % mColumns == 1) {
						lp.setMargins(com.psgod.Utils.dpToPx(mContext, 4),
								com.psgod.Utils.dpToPx(mContext, 4),
								com.psgod.Utils.dpToPx(mContext, 4), 0);
					} else {
						// 增加上下边距
						lp.setMargins(0, com.psgod.Utils.dpToPx(mContext, 4),
								0, 0);
					}
				}
				...
				if (mColumns == 2) {
					if (index % mColumns == 0) {
						...
						lp.setMargins(com.psgod.Utils.dpToPx(mContext, 4),
								com.psgod.Utils.dpToPx(mContext, 4), 0, 0);
					}
				}
