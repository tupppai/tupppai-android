...
public abstract class ExtendableListView extends AbsListView {
		...
		if (motionPosition >= 0) {
			...
			if (child != null && !child.hasFocusable()) {
				...
				if (mTouchMode == TOUCH_MODE_DOWN
					...
					return true;
				} else if (!mDataChanged && motionPosition >= 0
						&& mAdapter.isEnabled(motionPosition)) {
					post(performClick);
				}
			}
	...
	public int getLastVisiblePosition() {
		return Math.min(mFirstPosition + getChildCount() - 1,
				mAdapter != null ? mAdapter.getCount() - 1 : 0);
	}
		...
		public void run() {
			...
			if (adapter != null && mItemCount > 0
				...
				if (view != null) {
					...
					performItemClick(view, clickPosition,
	...
	private boolean performLongPress(final View child,
		...
		if (onItemLongClickListener != null) {
			handled = onItemLongClickListener.onItemLongClick(
					ExtendableListView.this, child, longPressPosition,
					longPressId);
		}
		// if (!handled) {
